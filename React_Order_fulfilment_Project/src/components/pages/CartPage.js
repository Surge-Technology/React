...
const CartPage = () => {
  ...
  const loadUser = async () => {
    ...
    const response = await axios.get(
      `http://localhost:8080/getAddToCartDetail/${loggedInUserId}`
    );
  ...
  const removeProduct = async (productId) => {
    ...
    if (confirmed) {
      ...
      const jsonData = {
        ...
        product_Qnty: cartItems.find((item) => item.product_ID === productId)
          .quantity,
      };
        ...
        const updatedCartItems = cartItems.map((cartItem) => {
          if (cartItem.product_ID === productId) {
            return {
              ...
              productList: cartItem.productList.filter(
